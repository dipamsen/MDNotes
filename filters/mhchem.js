        ...
        if (matches) {
          ...
          for (var iA = 0; iA < task.action_.length; iA++) {
            ...
            if (machine.actions[task.action_[iA].type_]) {
              ...
              o = _mhchemParser.actions[task.action_[iA].type_](
                buffer,
                matches.match_,
                task.action_[iA].option
              );
  ...
  patterns: {
    patterns: {
      ...
      "state of aggregation $": function (input) {
        var a = _mhchemParser.patterns.findObserveGroups(
          input,
          "",
          /^\([a-z]{1,3}(?=[\),])/,
          ")",
      ...
      "...": /^\.\.\.(?=$|[^.])/,
      "^{(...)}": function (input) {
        return _mhchemParser.patterns.findObserveGroups(
          input,
          "^{",
          "",
          "",
      ...
      "${(...)}$__$(...)$": function (input) {
        return (
          _mhchemParser.patterns.findObserveGroups(input, "${", "", "", "}$") ||
          _mhchemParser.patterns.findObserveGroups(input, "$", "", "", "$")
        );
      },
      "=<>": /^[=<>]/,
      ...
      "\\color{(...)}{(...)}": function (input) {
        return (
          _mhchemParser.patterns.findObserveGroups(
            input,
            "\\color{",
            "",
            "",
    ...
    ce: {
      ...
      actions: {
        ...
        "charge or bond": function (buffer, m) {
          if (buffer["beginsWithBond"]) {
            ...
            _mhchemParser.concatArray(
              ret,
              _mhchemParser.actions["bond"](buffer, m, "-")
            );
            return ret;
          } else {
            buffer.d = m;
        ...
        "- after o/d": function (buffer, m, isAfterD) {
          ...
          var c2 = _mhchemParser.patterns.match_(
            "one lowercase greek letter $",
            buffer.o || ""
          );
          var c3 = _mhchemParser.patterns.match_(
            "one lowercase latin letter $",
            buffer.o || ""
          );
          var c4 = _mhchemParser.patterns.match_(
            "$one lowercase latin letter$ $",
            buffer.o || ""
          );
          var hyphenFollows =
          ...
          if (hyphenFollows) {
            ...
            if (isAfterD && c1 && c1.remainder === "") {
              _mhchemParser.concatArray(
                ret,
                _mhchemParser.actions["d="](buffer, m)
              );
              _mhchemParser.concatArray(ret, this["output"](buffer));
            } else {
              _mhchemParser.concatArray(ret, this["output"](buffer));
            ...
            if (
              ...
              if (
                ...
                if (
                  buffer.o &&
                  buffer.dType === "kv" &&
                  _mhchemParser.patterns.match_("d-oxidation$", buffer.d || "")
                ) {
    ...
    switch (buf.type_) {
      ...
      case "color":
        res =
          ...
          buf.color1 +
          "}{" +
          _mhchemTexify._goInner(buf.color2) +
          "}}";
        break;
      case "color0":
        res = "\\color{" + buf.color + "}";
